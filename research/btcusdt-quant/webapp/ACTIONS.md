...
any branch except `main`. Until this file is merged to `main` it will never run
on its own — only the **Run workflow** button works, and that works from any
branch.
...
3. tops the panels up from Binance's **public** archive and REST — no API key is
   needed to compute a decision, only to place an order
