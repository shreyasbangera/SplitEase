...
## S127–S128 — Checking the sources properly. Two new data classes, both noise.

S126 recorded "no new data is reachable". **That conclusion was wrong**, and the correction matters
more than any result in this section. The container's egress is blocked and Binance 403s the
datacenter IP — both true — but this session also has MCP servers attached that do not use the
container's network at all. Four were checked:

| source | result |
|---|---|
| Crypto.com | works; **50 candles maximum**. Unusable for a backtest. |
| LunarCrush | works; **"limited data mode"** masks every value behind a subscription. Dates, no numbers. |
| **Twelve Data** | **works.** Up to 5,000 daily bars per symbol, equities/ETFs/forex/commodities. |
| **Firecrawl** | **works, and runs server-side so it reaches the blocked hosts.** Verified against Deribit's API: status 200, live JSON. |

So new data *is* obtainable from here. Both working routes were spent on what S124 says is needed —
a return stream generated by something other than crypto-market beta.

### S127 — macro risk appetite: nothing

Six series via Twelve Data (HYG, LQD, TLT, SPY, UUP, 1,830 trading days from 2019-06) joined to the
VIX history already on disk. Each verified against an expected price band rather than trusted by
call order. Macro closes at 21:00 UTC and BTC's daily bar at 00:00 UTC, so a reading stamped D is
known before BTC's D+1 bar; it is forward-filled onto the BTC grid and lagged a further full day,
with weekends carrying **Friday** forward — filling backward would hand the book Monday's panic on
Friday.

Eight features: credit spread level and change (HYG/LQD), equity momentum, VIX level and change,
dollar, long bond, and a stress composite. **Zero survived the screen.** Three kept their sign
across halves and were put through the full test anyway, since dismissing the strongest feature on a
threshold rather than a test would be sloppy:

| feature | any horizon excluding 0 | surrogate p |
|---|---|---|
| rates (TLT 20d) | none | 0.247 |
| dollar (UUP 20d) | none | 0.523 |
| VIX level | none | 0.387 |

### S128 — on-chain: the strongest ICs in the study, and still noise

Five series via Firecrawl from blockchain.info — miner revenue, hash rate, unique addresses, settled
USD value, mempool size — **2,921 days from 2018-09**, the longest sample anywhere in this study.
Six features with mechanisms stated first: hash ribbon, miner margin, NVT, address growth,
throughput, mempool congestion.

One survivor, and it produced the largest ICs recorded in this entire log:

| horizon | 1d | 3d | 5d | 10d | 20d |
|---|---|---|---|---|---|
| mempool IC | +0.0379 | +0.0628 | +0.0777 | +0.0932 | **+0.1419** |

Both halves positive (+0.0837, +0.0584), rising monotonically with horizon — the shape a real slow
signal makes. **And it is noise.** No horizon's bootstrap interval excludes zero once overlapping
windows are removed, and **79 of 300 phase-randomised surrogates match or beat it (p = 0.263)**.
Mempool size is strongly autocorrelated, and autocorrelated series spuriously track autocorrelated
forward returns; that is exactly the failure the surrogate control exists to detect, and it detected
it. Without that control this would have been written up as the best signal in the study.

### Where this leaves it

Two genuinely new data classes — the first inputs in this study generated outside the crypto market
entirely — both measured against proper nulls, both noise. S124's requirement is unchanged and
unmet: roughly three uncorrelated V7-quality streams, and this study has one.

The operational finding is worth more than the negative results: **the Firecrawl route is open**, so
data acquisition is no longer the blocker it was recorded as. What it cannot supply is bulk
historical *options* data — Deribit serves only ~15 days of volatility history publicly and no
